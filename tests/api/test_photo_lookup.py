...
def test_classify_pending_404_for_unscanned_asset(api_client):
    response = api_client.post("/photo-lookup/does-not-exist/classify-pending")

    assert response.status_code == 404


def test_classify_pending_creates_a_classification_for_a_pending_crop(
    api_client, engine
):
    # Issue #245: a Crop with no CropClassification yet (the classify stage
    # hasn't reached it) previously had no way to get species/identity
    # controls short of a full destructive "Repair". This endpoint should
    # classify it in place, without touching other detections' state.
    with Session(engine) as session:
        asset = Asset(immich_asset_id="asset-pending", extension=".jpg")
        detection = Detection(
            asset=asset, label="dog", confidence=0.9, x1=0, y1=0, x2=10, y2=10
        )
        Crop(detection=detection, path="pending-crop.jpg", species=Species.DOG)
        session.add(asset)
        session.commit()

    response = api_client.post("/photo-lookup/asset-pending/classify-pending")

    assert response.status_code == 200
    assert response.json()["classified"] == 1

    lookup = api_client.get("/photo-lookup/asset-pending").json()
    detection = lookup["detections"][0]
    assert detection["classification_id"] is not None


def test_classify_pending_leaves_other_detections_on_the_asset_untouched(
    api_client, engine
):
    with Session(engine) as session:
        asset = Asset(immich_asset_id="asset-mixed", extension=".jpg")

        classified_detection = Detection(
            asset=asset, label="dog", confidence=0.9, x1=0, y1=0, x2=10, y2=10
        )
        classified_crop = Crop(
            detection=classified_detection, path="classified.jpg", species=Species.DOG
        )
        CropClassification(crop=classified_crop, identity="Fibs", confidence=0.95)

        pending_detection = Detection(
            asset=asset, label="dog", confidence=0.9, x1=20, y1=20, x2=30, y2=30
        )
        Crop(detection=pending_detection, path="pending.jpg", species=Species.DOG)

        session.add(asset)
        session.commit()

    response = api_client.post("/photo-lookup/asset-mixed/classify-pending")

    assert response.status_code == 200
    assert response.json()["classified"] == 1

    lookup = api_client.get("/photo-lookup/asset-mixed").json()
    detections = {d["x1"]: d for d in lookup["detections"]}

    assert detections[0]["identity"] == "Fibs"
    assert detections[0]["confidence"] == 0.95
    assert detections[20]["classification_id"] is not None


def test_classify_pending_is_a_no_op_when_nothing_is_pending(api_client, engine):
    with Session(engine) as session:
        _seed_asset_with_detection(session)

    response = api_client.post("/photo-lookup/asset-1/classify-pending")
