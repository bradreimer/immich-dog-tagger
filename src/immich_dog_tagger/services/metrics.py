...
# this app's validated 7-color categorical chart palette (--chart-1..7,
# DT-1104 + issue #275's magenta/green addition: one band per top identity
# plus one for "Other"), not an arbitrary cutoff (issue #271).
TOP_N_TIMELINE_IDENTITIES = 6
...
class MetricsService:
    ...
    def species_timeline(self, species: Species) -> SpeciesTimeline:
        """
        Confirmed-photo volume for one species, bucketed by calendar year and
        stacked by individual pet identity (issue #271, year grain per #275)
        -- keyed off `PetOccurrence`/`Asset.captured_at`, the same "when was
        this photo taken" concept the Insights page's time-based facts
        already use (`MostActiveMonthProvider` et al.), not `Asset.created_at`
        (pipeline ingestion time, a different concept `_detection_coverage()`
        covers).

        One flat query, all bucketing/ranking done in Python: the row count
        here is bounded by "confirmed photos of this species" -- the same
        order of magnitude an InsightProvider already loads fully into Python
        for one identity, just for every identity of one species here
        instead.
        """
        ...
        counts_by_bucket: dict[int, Counter[tuple[int, str]]] = {}
        ...
        for captured_at, identity_id, identity_name in rows:
            ...
            counts_by_bucket.setdefault(year, Counter())[identity_key] += 1
        ...
        first_year, last_year = min(counts_by_bucket), max(counts_by_bucket)
        ...
        for year in all_years:
            ...
            points.append(SpeciesTimelinePoint(label=str(year), counts=counts))
